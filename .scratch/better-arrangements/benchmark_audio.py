...
with tempfile.TemporaryDirectory() as directory:
    ...
    with sf.SoundFile(path, mode="w", samplerate=sr, channels=2, subtype="PCM_16") as output:
        for start in range(0, seconds, 30):
            ...
            signal = (0.15 * np.sin(2 * np.pi * 220 * t) + 0.08 * np.sin(2 * np.pi * 330 * t)) * (
                0.6 + 0.4 * np.cos(4 * np.pi * t)
            )
            output.write(np.column_stack((signal, signal)))
    audio_analysis.analyze_sections([("full", 0.0, 1.0, np.zeros(audio_analysis.SAMPLE_RATE, dtype=np.float32))])
    begin = time.perf_counter()
    wave, sample_rate = audio_analysis.load_audio(str(path))
    decoded = time.perf_counter()
    sections = [
        (label, start, end, wave[int(start * sample_rate) : int(end * sample_rate)])
        for label, start, end in audio_analysis.section_plan(len(wave) / sample_rate)
    ]
    features = audio_analysis.analyze_sections(sections, sample_rate)
    analyzed = time.perf_counter()
    assert features["duration"] == seconds
    ...
    print(
        json.dumps(
            {
                "seconds": seconds,
                "decode_seconds": round(decoded - begin, 4),
                "analysis_seconds": round(analyzed - decoded, 4),
                "warm_cache_median_seconds": round(statistics.median(timings), 5),
                "peak_process_mib": round(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024, 1),
                "cache_bytes": cache_bytes,
                "python": platform.python_version(),
                "machine": platform.machine(),
                "logical_cpus": os.cpu_count(),
                "analysis_version": audio_analysis.ANALYSIS_VERSION,
            }
        )
    )
