...
| Chroma | `librosa.feature.chroma_stft(S=magnitude**2, sr=sr, tuning=None, norm=2)` accepts a power spectrogram. Default `base_c=True` orders bins C, C#, D, D#, E, F, F#, G, G#, A, A#, B. `tuning=None` estimates the deviation from A440 per block, so uploads recorded flat or sharp keep coherent chroma instead of smearing across bins; a fixed `tuning=0` collapsed key evidence past roughly 35 cents of detune in fixtures. `chroma_cqt` was measured and rejected: its log-frequency bins fix STFT's sub-C3 semitone smear, but on generated triad fixtures its key-profile correlation halved (about 0.46 versus 0.91 across detunings and variants with narrower octave ranges), which would push borderline recordings below the key-evidence gate. Chroma is tonal content, not a key label. [Chroma](https://librosa.org/doc/1.0.0/api/generated/librosa.feature.chroma_stft.html) |
...
Evidence strengths are heuristics, not probabilities. Single pitches do not receive major/minor key labels; silence and generated aperiodic noise do not receive tempo estimates. The rhythm measurement removes the onset activity baseline before checking autocorrelation peaks, then retains competing tempo candidates and reduces ambiguous evidence. When sampled windows merge, near-identical candidate peaks within one BPM combine, and the summary beat follows the windows' own half/double decisions — an evidence-weighted majority over their estimate-arbitrated choices — because the pace tilt compares log2 tempos across tracks and an octave slip would move a song to the wrong end. Sparse sections raise stronger half-tempo peaks than the true beat, so the strongest raw peak is only a fallback when no window could choose. Short excerpts have less support. Raw level/activity measurements stay in the cache; intensity normalization happens per playlist, and display energy treats unmeasured components as neutral.
